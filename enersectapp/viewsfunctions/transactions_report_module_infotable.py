...
def transactions_report(request):
    ...
    with transaction.commit_on_success():
        for item in searchtags:
            ...
            if item["tag_name"] == "Unique Transaction Index":
                ...
                searchtags_used_list.append("Unique Transaction Index")
            ...
            if item["tag_name"] == "Amount":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))
                searchtags_used_list.append("Amount")
            ...
            if item["tag_name"] == "Piece Number":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))
                searchtags_used_list.append("Piece Number")
            ...
            if item["tag_name"] == "Libdesc":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))
                searchtags_used_list.append("Libdesc")
            ...
            if item["tag_name"] == "Company":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))
                searchtags_used_list.append("Company")
            ...
            if item["tag_name"] == "Day":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))
                searchtags_used_list.append("Day")
            ...
            if item["tag_name"] == "Month":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))
                searchtags_used_list.append("Month")
            ...
            if item["tag_name"] == "Year":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))
                searchtags_used_list.append("Year")
            ...
            if item["tag_name"] == "Complete Date":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))
                searchtags_used_list.append("Complete Date")
            ...
            if item["tag_name"] == "Reftran":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))
                searchtags_used_list.append("Reftran")
            ...
            if item["tag_name"] == "Description":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))
                searchtags_used_list.append("Description")
            ...
            if item["tag_name"] == "Bank Name":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))
                searchtags_used_list.append("Bank Name")
            ...
            if item["tag_name"] == "Bank Account":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))
                searchtags_used_list.append("Bank Account")
            ...
            if item["tag_name"] == "Bank Currency":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))
                searchtags_used_list.append("Bank Currency")
            ...
            if item["tag_name"] == "INT Account Number":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))  
                searchtags_used_list.append("INT Account Number")
            ...
            if item["tag_name"] == "Exchange Rate":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))
                searchtags_used_list.append("Exchange Rate")
            ...
            if item["tag_name"] == "Memo":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))  
                searchtags_used_list.append("Memo")
            ...
            if item["tag_name"] == "Movement Number":
                ...
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True))  
                searchtags_used_list.append("Movement Number")
                
            if item["tag_name"] == "Lett":
            
                coincident_transactions = TransactionTable.objects.filter(internal_records_list__NoMvt__contains = item["tag_content"]).order_by()
                coincident_transactions_list.extend(coincident_transactions.values_list('TransactionIndex',flat=True)) 
                searchtags_used_list.append("Lett")
    ...
    searchtags_used_list = set(searchtags_used_list)
    searchtags_used_list = list(searchtags_used_list)
    ...
    for dict_key in coincident_transactions_dict.keys():
        ...
        ##Adding to the dictionary of the candidate the fields that appear in the selected tags
        
        if "Amount" in searchtags_used_list:
            temp_dict["Amount"] = transaction_item.Amount
            
        if "Piece Number" in searchtags_used_list:
        
            piece_num_string = ""
    
            for single_item in transaction_item.internal_records_list.all().values("NoPiece"):
        
                piece_num_string += "| "
                piece_num_string += single_item["NoPiece"]
                piece_num_string += " "
        
            temp_dict["Piece Number"] = piece_num_string
            
        if "Company" in searchtags_used_list:
            
            company_string = ""
    
            for single_item in transaction_item.internal_records_list.all().values("Company"):
        
                company_string += "| "
                company_string += single_item["Company"]
                company_string += " "
            
            temp_dict["Company"] = company_string
        
        if "Day" in searchtags_used_list:
        
            day_string = "| " + transaction_item.PostDay + " " + "|" + transaction_item.ValueDay + ""
        
            temp_dict["Day"] = day_string
            
        if "Month" in searchtags_used_list:
        
            month_string = "| " + transaction_item.PostMonth + " " + "|" + transaction_item.ValueMonth + ""
        
            temp_dict["Month"] = month_string
            
        if "Year" in searchtags_used_list:
        
            year_string = "| " + transaction_item.PostYear + " " + "|" + transaction_item.ValueYear + ""
        
            temp_dict["Year"] = year_string
            
        if "Complete Date" in searchtags_used_list:
            
            complete_date_string = "| " + transaction_item.PostDay + transaction_item.PostMonth + transaction_item.PostYear + " " + "|" + transaction_item.ValueDay + transaction_item.ValueMonth + transaction_item.ValueYear + ""
        
            temp_dict["Complete Date"] = complete_date_string
            
        if "Libdesc" in searchtags_used_list:
        
            temp_dict["Libdesc"] = transaction_item.Libdesc
            
        if "Description" in searchtags_used_list:
        
            description_string = ""
    
            for single_item in transaction_item.bank_records_list.all().values("Description"):
        
                description_string += "| "
                description_string += single_item["Description"]
                description_string += " "
        
            temp_dict["Description"] = description_string
            
        if "Reftran" in searchtags_used_list:
            temp_dict["Reftran"] = transaction_item.Reftran
            
        if "Bank Name" in searchtags_used_list:
            temp_dict["Bank Name"] = transaction_item.BankName
            
        if "Bank Account" in searchtags_used_list:
            temp_dict["Bank Account"] = transaction_item.BankAccount
            
        if "Bank Currency" in searchtags_used_list:
            temp_dict["Bank Currency"] = transaction_item.BankCurrency
            
        if "Movement Number" in searchtags_used_list:
        
            movementnumber_string = ""
    
            for single_item in transaction_item.internal_records_list.all().values("NoMvt"):
        
                movementnumber_string += "| "
                movementnumber_string += single_item["NoMvt"]
                movementnumber_string += " "
        
            temp_dict["Movement Number"] = movementnumber_string
            
        if "INT Account Number" in searchtags_used_list:
        
            intaccountnumber_string = ""
    
            for single_item in transaction_item.internal_records_list.all().values("AccountNum"):
        
                intaccountnumber_string += "| "
                intaccountnumber_string += single_item["AccountNum"]
                intaccountnumber_string += " "
        
            temp_dict["INT Account Number"] = intaccountnumber_string
            
        if "Exchange Rate" in searchtags_used_list:
        
            exchangerate_string = ""
    
            for single_item in transaction_item.internal_records_list.all().values("ExchangeRate"):
        
                exchangerate_string += "| "
                exchangerate_string += single_item["ExchangeRate"]
                exchangerate_string += " "
        
            temp_dict["Exchange Rate"] = exchangerate_string
            
        if "Memo" in searchtags_used_list:
            
            memo_string = ""
    
            for single_item in transaction_item.internal_records_list.all().values("Memo"):
        
                memo_string += "| "
                memo_string += single_item["Memo"]
                memo_string += " "
        
            temp_dict["Memo"] = memo_string
            
        if "Lett" in searchtags_used_list:
        
            lett_string = ""
    
            for single_item in transaction_item.internal_records_list.all().values("Lett"):
        
                lett_string += "| "
                lett_string += single_item["Lett"]
                lett_string += " "
        
            temp_dict["Lett"] = lett_string
